...
def get_valid_token(client, admin=False):
...
def get_token_inactive_user(client):
    add_user('test', 'test@test.com', 'test')
    user = User.query.filter_by(email='test@test.com').first()
    user.active = False
    db.session.commit()
...
class TestUserService(BaseTestCase):
    ...
    def test_add_user(self):
        ...
        with self.client:
            token = get_valid_token(self.client, admin=True)
    ...
    def test_add_user_inactive(self):
        with self.client:
            token = get_token_inactive_user(self.client)
            response = self.client.post(
                '/users',
                data=json.dumps({
                    'username': 'michael',
    ...
    def test_add_user_not_admin(self):
        with self.client:
            token = get_valid_token(self.client)
            response = self.client.post(
                '/users',
                data=json.dumps({
                    'username': 'michael',
